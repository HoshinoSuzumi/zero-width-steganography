import { ZWSCharSet, ZWSCharacter, ZWSDecoded } from "./types";

let chars: ZWSCharSet = [];
let radix = 0;
let codeLengthText = 0;
let codeLengthBinary = 0;

const setCharSet = (newChars: ZWSCharSet): void => {
...
const encodeText = (text1: string, text2: string): string => {
  return combine_shuffle_string(
    text1,
    encode_to_zw_characters_text(text2),
    codeLengthText
  );
};

const encodeBinary = (text: string, data: Uint8Array): string => {
  return combine_shuffle_string(
    text,
    encode_to_zw_characters_binary(data),
    codeLengthBinary
  );
};

const decodeText = (text: string): ZWSDecoded<string> => {
  let split = split_zw_characters(text);
  return {
    original: split.originalText,
    hidden: decode_from_zero_width_characters_text(split.hiddenText),
  };
};

const decodeBinary = (text: string): ZWSDecoded<Uint8Array> => {
  let split = split_zw_characters(text);
  return {
    original: split.originalText,
    hidden: decode_from_zw_characters_binary(split.hiddenText),
  };
};

const encode_to_zw_characters_text = (str1: string) => {
...
const encode_to_zw_characters_binary = (u8ary: Uint8Array) => {
...
const combine_shuffle_string = function (
  str1: string,
  str2: string,
  codeLength: number
) {
...
const split_zw_characters = (
  ...
  result.originalText = str1.replace(
    new RegExp("[" + chars.join("") + "]", "g"),
    ""
  );
  result.hiddenText = str1.replace(
    new RegExp("[^" + chars.join("") + "]", "g"),
    ""
  );
  return result;
...
const decode_from_zero_width_characters_text = (str1: string) => {
  let r = str1;
  let i;
  let result = [];
  for (i = 0; i < radix; i++) {
...
const decode_from_zw_characters_binary = (str1: string) => {
  let r = str1;
  let i;
  let j;
...
const ZW_CHARACTERS = Object.freeze<ZWSCharacter[]>([
  {
  ...
  {
    label: "ZERO WIDTH NO-BREAK SPACE",
    character: "\ufeff",
  },
]);
